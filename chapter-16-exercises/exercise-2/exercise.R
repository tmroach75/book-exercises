...
ggplot(data = diamonds) +
  geom_col(mapping = aes(x = price, y = cut))

# Draw the same chart again, but with each element positioned to "fill" the y axis

ggplot(data = diamonds) +
  geom_col(mapping = aes(x = price, y = cut, fill = cut))

# Draw the same chart again, but with each element positioned to "dodge" each other

ggplot(data = diamonds) +
  geom_col(mapping = aes(x = price, y = cut, fill = cut), position = "dodge"
)
...
ggplot(data = diamond_sample) +
  geom_boxplot(mapping = aes(x = color, y = price)) + scale_x_log10(limits = range(diamonds$price))
...
bar_chart <- ggplot(data = diamond_sample) +
  geom_col(mapping = aes(x = cut, y = price, fill = cut))

# Draw the same chart, but with the coordinate system flipped

ggplot(data = diamond_sample) +
  geom_col(mapping = aes(x = price, y = cut, fill = price))
