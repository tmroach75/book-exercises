...
View(arrange(flights, delayed_in_air))
View(arrange(flights, desc(delayed_in_air)))

# For practice, repeat the last 2 steps in a single statement using the pipe
# operator. You can clear your environmental variables to "reset" the data frame

flights <- flights %>% mutate(delayed_in_air = arr_delay - dep_delay) %>% arrange(desc(delayed_in_air))
